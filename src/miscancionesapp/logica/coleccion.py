...
class Coleccion():
    ...
    def agregar_cancion(self, titulo, minutos, segundos, compositor) :
        ...
        def agregar_interprete(self, nombre, textodescripcion):
            busqueda = session.query(Interprete).filter(Interprete.nombre == nombre).all()
            if len(busqueda) == 0:
                interprete = Album(nombre=nombre, texto_curiosidades=textodescripcion)
        ...
        def editar_interprete(self, interprete_id, nombre, textodescripcion):
            busqueda = session.query(Interprete).filter(Interprete.nombre == nombre, Interprete.id != interprete_id).all()
            if len(busqueda) == 0:
                album = session.query(Interprete).filter(Interprete.id == interprete_id).first()
                album.nombre = nombre
                album.textodescripcion = textodescripcion
                session.commit()
                return True
            else:
                return False
